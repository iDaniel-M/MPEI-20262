...
// Funcion para limpiar la matriz y empezar el juego de cero
...
// Funcion para consultar que jugador tiene una celda en especifico
uint8_t obtener_casilla(uint8_t fila, uint8_t col) {
    ...
    return CASILLA_VACIA; // Retorno de seguridad ante indices no validos
}
...
uint8_t evaluar_estado_juego(void) {
    ...
    for (uint8_t x = 0; x < 3; x++) {
        ...
        // Chequeo de columnas (revision vertical)
    ...
    // 3. Verificamos si hay empate contando las casillas vacias (logica cambiada frente al original)
    ...
    // Si ya no quedan huecos y ninguna validacion anterior dio un ganador, es empate
    if (casillas_vacias == 0) {
        return ESTADO_EMPATE;
    }

    // Si la matriz no esta llena y nadie ha ganado, se sigue jugando
    return ESTADO_ACTIVO;
}

// Inteligencia artificial basica para la maquina
void turno_maquina(void) {
    // Verificacion de seguridad para no alterar la memoria de una partida terminada
    ...
    for (int8_t f = 2; f >= 0; f--) {
        for (int8_t c = 2; c >= 0; c--) {
            ...
            if (tablero[f][c] == CASILLA_VACIA) {
                tablero[f][c] = JUGADOR_DOS; // Ejecutamos la jugada de la maquina
                estado_actual = evaluar_estado_juego(); // Revalidamos la condicion de victoria
